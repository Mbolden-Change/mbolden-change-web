...
export default function FiftyFifty({
  ...
  const getAspectRatioClass = () => {
    ...
    switch (imageAspectRatio) {
      case '16:9':
        return styles['aspectRatio-16-9'];
      case '4:3':
        return styles['aspectRatio-4-3'];
      case '1:1':
        return styles['aspectRatio-1-1'];
      case '9:16':
        return styles['aspectRatio-9-16'];
      default:
        return '';
    }
  };
  ...
  const VideoPlayer = ({ url, title }: { url: string; title: string }) => (
    <div className={styles.videoWrapper}>
      {videoTitle && (
        <Headline tag="h2" text={videoTitle} className={styles.videoTitle} />
      )}
      <iframe
        ...
        <Grid>
          <GridItem desktopSpan={6}>
            {mediaType === 'video' && leftVideoUrl ? (
              ...
              <div
                className={`${styles.imageWrapper} ${getAspectRatioClass()}`}
              >
            ...
            {leftText && (
              <PortableTextComponent value={leftText as PortableTextBlock[]} />
            )}
          </GridItem>
              ...
              <div className={styles.fiftyFiftyMobileTextContainer}>
                ...
                {leftText && (
                  <PortableTextComponent
                    value={leftText as PortableTextBlock[]}
                  />
                )}
                {rightTitle && <Headline tag="h2" text={rightTitle} />}
                {rightText && (
                  <PortableTextComponent
                    value={rightText as PortableTextBlock[]}
                  />
                )}
              </div>
            </>
