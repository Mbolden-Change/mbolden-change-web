...
const PopUpModal = ({ popUpModalData }: PopUpModalProps) => {
  ...
  const [isActionNetworkModalOpen, setIsActionNetworkModalOpen] =
    useState(false);
  ...
  const isActionNetworkUrl = (url?: string) =>
    url?.includes(
      'https://actionnetwork.org/forms/sign-up-to-get-the-latest-from-mbolden-change'
    );
  ...
  if (popUpModalData.visibility) {
    return (
      ...
      <div
        className={`${modalState === 'collapsed' ? styles.minimized : styles.modalWrapper}`}
      >
          ...
          <>
            <div className={styles.modalContent}>
              ...
              <div className={styles.topText}>
                {popUpModalData.title ? (
                  <Headline
                    tag="h3"
                    text={popUpModalData.title || ''}
                    className={styles.headline}
                  />
                ) : null}
                {popUpModalData.body ? (
                  <p className={styles.body}>{popUpModalData.body || ''}</p>
                ) : null}
              </div>
              {popUpModalData.image ? (
